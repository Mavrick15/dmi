...
const UserModal = ({ isOpen, onClose, user = null, onSave }) => {
  ...
  const [formData, setFormData] = useState({
    nomComplet: '',
    email: '',
    role: 'docteur_clinique', // Valeur par défaut
    department: '',
    phone: '',
    address: '',
    status: 'Active',
    ...
    if (isOpen) {
      ...
      if (user) {
        ...
        setFormData({
          ...
          email: user.email || '',
          role: user.role || 'docteur_clinique',
        ...
        setFormData({
          nomComplet: '', email: '', role: 'docteur_clinique', department: '', phone: '', address: '',
    ...
    <AnimatedModal isOpen={isOpen} onClose={onClose}>
      <div className="glass-panel w-full max-w-2xl shadow-xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-8 overflow-y-auto custom-scrollbar space-y-8">
          ...
          {(formData.role === 'docteur_clinique' || formData.role === 'docteur_labo') && (
            <section className="space-y-4">
              <h3 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-2">
