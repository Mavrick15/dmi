...
const ChartContainer = ({
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && Array.isArray(payload) && payload.length > 0 && payload[0]) {
      ...
      return (
        <div className="glass-dropdown p-4 min-w-[200px]">
  ...
  if (!Array.isArray(data) || data.length === 0 || loading) {
    return (
      <div className={`glass-panel p-6 ${className}`}>
         ...
         <div className={`flex flex-col items-center justify-center h-[200px] rounded-xl glass-surface ${loading ? 'border-l-4 border-l-primary' : ''}`}>
             {loading ? (
  ...
  return (
    <div className={`glass-panel p-6 transition-colors duration-300 ${className}`}>
