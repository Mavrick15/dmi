...
const RecentOrdersTable = ({ orders, loading, onReceiveOrder, onViewOrderDetails, onViewHistory }) => {
  ...
  if (loading) {
     return <div className="flex justify-center items-center h-48 text-primary"><Icon name="Loader2" className="animate-spin" size={32} /></div>;
  }
  
  if (orders.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center h-48 text-slate-400 bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">
           <Icon name="ShoppingCart" size={32} />
           <p className="mt-2 text-sm">Aucune commande récente trouvée.</p>
           <p className="text-xs text-slate-400">Utilisez le bouton "Nouvelle commande" dans l'onglet Inventaire.</p>
        </div>
      );
  }
