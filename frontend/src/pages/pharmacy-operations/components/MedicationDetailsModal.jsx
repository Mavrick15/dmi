...
const MedicationDetailsModal = ({ isOpen, onClose, medication }) => {
  ...
  const modalContent = (
    <AnimatedModal isOpen={isOpen} onClose={onClose} usePortal={true}>
      <div className="bg-white dark:bg-slate-900 w-full max-w-4xl max-h-[85vh] my-auto rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-slate-200 dark:border-slate-800">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row gap-6 items-center sm:items-start relative">
          ...
          <div className="relative flex-shrink-0">
            <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl bg-gradient-to-br from-primary/20 to-blue-600/20 dark:from-primary/30 dark:to-blue-600/30 flex items-center justify-center border-4 border-white dark:border-slate-800 shadow-lg">
              <Icon name="Pill" size={48} className="text-primary dark:text-blue-400" />
            </div>
            <span className={`absolute -bottom-2 -right-2 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white rounded-full ${stockStatus.color}`}>
              {stockStatus.label}
            </span>
          </div>

          <div className="flex-1 text-center sm:text-left space-y-1">
            <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white">{data.nom}</h2>
            <div className="flex flex-wrap justify-center sm:justify-start items-center gap-3 text-sm text-slate-500 dark:text-slate-400">
              <span className="flex items-center gap-1 font-mono bg-slate-100 dark:bg-slate-800 px-1.5 rounded">
        ...
        {/* Contenu Principal */}
        <div className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar bg-slate-50/30 dark:bg-black/20">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl p-5 shadow-sm">
              ...
              <div className="space-y-4 text-sm">
                <div className="flex items-start gap-3">
                  <div className="mt-0.5 text-slate-400"><Icon name="FlaskConical" size={16} /></div>
                  <div>
                    <p className="text-xs text-slate-400">Principe actif</p>
                    <p className="font-medium text-slate-700 dark:text-slate-200">{data.principeActif}</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <div className="mt-0.5 text-slate-400"><Icon name="Tag" size={16} /></div>
                  <div>
                    <p className="text-xs text-slate-400">Catégorie</p>
                    <p className="font-medium text-slate-700 dark:text-slate-200 capitalize">{data.categorie}</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <div className="mt-0.5 text-slate-400"><Icon name="Building2" size={16} /></div>
                  <div>
                    <p className="text-xs text-slate-400">Fabricant</p>
                    <p className="font-medium text-slate-700 dark:text-slate-200">{data.fabricant}</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <div className="mt-0.5 text-slate-400"><Icon name="DollarSign" size={16} /></div>
                  <div>
                    <p className="text-xs text-slate-400">Prix unitaire</p>
                    <p className="font-medium text-slate-700 dark:text-slate-200">
                      {formatCurrency(data.prixUnitaire || 0)}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">
                    ...
                    <p className="text-lg font-bold text-slate-900 dark:text-white">{data.stockMinimum}</p>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">
                    <p className="text-xs text-slate-500 dark:text-slate-400">Stock maximum</p>
                    <p className="text-lg font-bold text-slate-900 dark:text-white">{data.stockMaximum}</p>
                  </div>
                </div>
                {data.dateExpiration && (
                  <div className="flex items-start gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
                    <div className="mt-0.5 text-slate-400"><Icon name="Calendar" size={16} /></div>
                    <div>
            ...
            {data.description && data.description !== 'Aucune description disponible' && (
              <div className="md:col-span-2 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl p-5 shadow-sm">
                <h4 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-4 flex items-center gap-2">
