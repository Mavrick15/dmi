...
router.group(() => {
  ...
  router.group(() => {
    ...
    router.post('/auth/unlock/:userId', [AuthController, 'unlockAccount'])
      .use(middleware.permission(['user_manage']))
    router.get('/auth/security-status/:email', [AuthController, 'getSecurityStatus'])
      .use(middleware.permission(['user_manage']))
    
    // Dashboard (accessible uniquement aux administrateurs et gestionnaires)
    ...
    router.post('/documents/:id/share', [DocumentsController, 'share'])
      .use(middleware.permission(['document_view']))
      .use(middleware.rateLimit({ maxRequests: 20, windowMs: 60 * 1000 })) // 20 req/min
    router.post('/documents/:id/revoke-access', [DocumentsController, 'revokeAccess'])
      .use(middleware.permission(['document_view']))
      .use(middleware.rateLimit({ maxRequests: 20, windowMs: 60 * 1000 })) // 20 req/min
    ...
    router.patch('/appointments/:id/status', [RendezVousController, 'updateStatus'])
      ...
      .use(middleware.rateLimit({ maxRequests: 50, windowMs: 60 * 1000 })) // 50 req/min
    router.patch('/appointments/:id', [RendezVousController, 'update'])
      .use(middleware.permission(['appointment_edit']))
      .use(middleware.rateLimit({ maxRequests: 50, windowMs: 60 * 1000 })) // 50 req/min
    ...
    router.group(() => {
      ...
      router.get('/stats', [AnalysesController, 'stats'])
      ...
      router.post('/', [AnalysesController, 'store'])
      ...
      router.put('/:id', [AnalysesController, 'update'])
        .use(middleware.permission(['analyses_edit']))
        .use(middleware.rateLimit({ maxRequests: 30, windowMs: 60 * 1000 })) // 30 req/min
      
      // Annuler une analyse - nécessite analyses_cancel
      router.patch('/:id/cancel', [AnalysesController, 'cancel'])
      ...
      router.group(() => {
        ...
        router.get('/', [ResultatsAnalyseController, 'index'])
        ...
        router.post('/', [ResultatsAnalyseController, 'store'])
    ...
    router.group(() => {
      ...
      router.put('/:id', [ResultatsAnalyseController, 'update'])
        .use(middleware.permission(['resultats_edit']))
        .use(middleware.rateLimit({ maxRequests: 30, windowMs: 60 * 1000 }))
      
      // Valider un résultat
      router.patch('/:id/validate', [ResultatsAnalyseController, 'validate'])
    ...
    router.group(() => {
      ...
      router.get('/knowledge', [KnowledgeBasesController, 'index'])
        .use(middleware.permission(['clinical_view']))
        .use(middleware.rateLimit({ maxRequests: 100, windowMs: 60 * 1000 })) // 100 req/min
      router.get('/knowledge/:id', [KnowledgeBasesController, 'show'])
        .use(middleware.permission(['clinical_view']))
        .use(middleware.rateLimit({ maxRequests: 100, windowMs: 60 * 1000 })) // 100 req/min
      router.post('/knowledge', [KnowledgeBasesController, 'store'])
        .use(middleware.permission(['settings_manage']))
        .use(middleware.rateLimit({ maxRequests: 20, windowMs: 60 * 1000 })) // 20 req/min
      router.put('/knowledge/:id', [KnowledgeBasesController, 'update'])
        .use(middleware.permission(['settings_manage']))
        .use(middleware.rateLimit({ maxRequests: 30, windowMs: 60 * 1000 })) // 30 req/min
      router.delete('/knowledge/:id', [KnowledgeBasesController, 'destroy'])
        .use(middleware.permission(['settings_manage']))
        .use(middleware.rateLimit({ maxRequests: 20, windowMs: 60 * 1000 })) // 20 req/min

      // Symptômes communs
      router.get('/symptoms', [CommonSymptomsController, 'index'])
        .use(middleware.permission(['clinical_view']))
        .use(middleware.rateLimit({ maxRequests: 100, windowMs: 60 * 1000 })) // 100 req/min
      router.post('/symptoms/:id/use', [CommonSymptomsController, 'incrementUsage'])
        .use(middleware.permission(['clinical_view']))
        .use(middleware.rateLimit({ maxRequests: 200, windowMs: 60 * 1000 })) // 200 req/min

      // Examens communs
      router.get('/exams', [CommonExamsController, 'index'])
        .use(middleware.permission(['clinical_view']))
        .use(middleware.rateLimit({ maxRequests: 100, windowMs: 60 * 1000 })) // 100 req/min
      router.post('/exams/:id/use', [CommonExamsController, 'incrementUsage'])
        .use(middleware.permission(['clinical_view']))
        .use(middleware.rateLimit({ maxRequests: 200, windowMs: 60 * 1000 })) // 200 req/min
    }).prefix('clinical')
    ...
    router.group(() => {
      ...
      // Prescriptions
      router.get('/prescriptions/pending', [PharmacyController, 'pendingPrescriptions'])
        .use(middleware.permission(['inventory_view']))
        .use(middleware.rateLimit({ maxRequests: 100, windowMs: 60 * 1000 })) // 100 req/min
      ...
      router.group(() => {
        ...
        router.patch('/prescriptions/:id/deliver', [PharmacyController, 'markPrescriptionDelivered'])
          .use(middleware.permission(['inventory_manage']))
          .use(middleware.rateLimit({ maxRequests: 50, windowMs: 60 * 1000 })) // 50 req/min
        router.patch('/prescriptions/:id', [PharmacyController, 'updatePrescription'])
          .use(middleware.permission(['inventory_manage']))
          .use(middleware.rateLimit({ maxRequests: 50, windowMs: 60 * 1000 })) // 50 req/min
    ...
    router.group(() => {
      ...
      router.get('/invoices/:id/pdf', [FinanceController, 'exportPdf'])
        .use(middleware.permission(['billing_view']))
        .use(middleware.rateLimit({ maxRequests: 50, windowMs: 60 * 1000 })) // 50 req/min
      
      // Récupérer une facture par ID - doit être AVANT /invoices (route spécifique avant route générale)
      router.get('/invoices/:id', [FinanceController, 'show'])
        .use(middleware.permission(['billing_view']))
        .use(middleware.rateLimit({ maxRequests: 100, windowMs: 60 * 1000 })) // 100 req/min
      ...
      router.post('/invoices/:id/refund', [FinanceController, 'refund'])
        .use(middleware.permission(['finance_manage']))
        .use(middleware.rateLimit({ maxRequests: 20, windowMs: 60 * 1000 })) // 20 req/min
      ...
      router.post('/invoices/:id/payment-plan', [FinanceController, 'createPaymentPlan'])
        .use(middleware.permission(['finance_manage']))
        .use(middleware.rateLimit({ maxRequests: 20, windowMs: 60 * 1000 })) // 20 req/min
      
      // Demandes d'assurance - nécessite finance_manage
      router.post('/invoices/:id/insurance-claim', [FinanceController, 'createInsuranceClaim'])
        .use(middleware.permission(['finance_manage']))
        .use(middleware.rateLimit({ maxRequests: 20, windowMs: 60 * 1000 })) // 20 req/min
      router.put('/insurance-claims/:claimId', [FinanceController, 'updateInsuranceClaim'])
        .use(middleware.permission(['finance_manage']))
        .use(middleware.rateLimit({ maxRequests: 30, windowMs: 60 * 1000 })) // 30 req/min
      ...
      router.get('/payment-methods', [PaymentMethodsController, 'index'])
        .use(middleware.permission(['billing_view']))
        .use(middleware.rateLimit({ maxRequests: 100, windowMs: 60 * 1000 })) // 100 req/min
      router.get('/payment-methods/stats', [PaymentMethodsController, 'stats'])
        .use(middleware.permission(['billing_view']))
        .use(middleware.rateLimit({ maxRequests: 50, windowMs: 60 * 1000 })) // 50 req/min
    }).prefix('finance')
    ...
    router.post('/users/:id/grant-permission', [UsersController, 'grantPermission'])
      .use(middleware.permission(['admin']))
      .use(middleware.rateLimit({ maxRequests: 30, windowMs: 60 * 1000 })) // 30 req/min
    router.post('/users/:id/revoke-permission', [UsersController, 'revokePermission'])
      .use(middleware.permission(['admin']))
      .use(middleware.rateLimit({ maxRequests: 30, windowMs: 60 * 1000 })) // 30 req/min
    
    // Gestion des sessions - nécessite admin
    router.post('/users/:id/expire-session', [UsersController, 'expireSession'])
      .use(middleware.permission(['admin']))
      .use(middleware.rateLimit({ maxRequests: 20, windowMs: 60 * 1000 })) // 20 req/min
    
    // Liste blanche IP - nécessite admin
    router.post('/ip-whitelist', [UsersController, 'updateIpWhitelist'])
      .use(middleware.permission(['admin']))
      .use(middleware.rateLimit({ maxRequests: 20, windowMs: 60 * 1000 })) // 20 req/min
    ...
    router.post('/audit', [AuditLogsController, 'store'])
    ...
    }).prefix('permissions').use(middleware.permission(['permission_manage']))
    ...
    }).prefix('stats').use(middleware.permission(['dashboard_view']))
    ...
    router.group(() => {
      ...
      router.get('/users', [ExportController, 'exportUsers'])
        .use(middleware.permission(['user_view']))
      router.get('/establishments', [ExportController, 'exportEstablishments'])
        .use(middleware.permission(['settings_manage']))
